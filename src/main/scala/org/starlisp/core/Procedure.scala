...
abstract class Procedure(val name : String = "", val minArgs: Int = 0, val maxArgs: Int = Integer.MAX_VALUE) extends LispObject {
  ...
  final def applyArgs(o: Array[LispObject]): LispObject = {
    if (o.length < minArgs) throw new LispException(Starlisp.internalError, "Too few args when calling procedure: " + toString)
    if (o.length > maxArgs) throw new LispException(Starlisp.internalError, "Too many args when calling procedure: " + toString)
    return apply(o)
  }
